...
<!-- 
    FIXME: This example is *very* similar to borrowck/2 although subtly different
        as it has to do with the desugaring of APITs rather than exclusively a
        universe error.

        This question should be moved into the *solution* to borrowck/2 as
        a "related example" that we reject before borrow checking due to the same
        underlying concept
 -->
